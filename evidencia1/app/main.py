...
def intentar_nuevamente_captcha():
    while True:
        opcion = input(f"¿Deseas intentar nuevamente (s) o salir del registro (x)?: ").strip().lower()
        if opcion == 's':
            return True
        elif opcion == 'x':
            print("Has decidido salir del registro.")
            return False
        else:
            print("Opción no válida. Por favor ingresa 's' para intentar nuevamente o 'x' para salir.")
...
def captcha(max_intentos=4):
    ...
    while intentos < max_intentos:
        try:
            ...
            if resultado == round(aritmetica.sumar(a, b), 2):
                print("¡Correcto!")
                return True
            else:
                intentos += 1
                ...
                if intentos < max_intentos:
                    if not intentar_nuevamente_captcha():
                        return False
        except ValueError:
            intentos += 1
